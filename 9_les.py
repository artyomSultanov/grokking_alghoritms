...
def get_selected_items(stuffdict, A=2000):
	...
	for search in items_list:
		for key, value in stuffdict.items():
			if value == search:
				selected_stuffdict.append(key)

	return selected_stuffdict

def get_memtable(stuffdict, A=2000):
	area, value = get_area_and_value(stuffdict)
	n = len(value) # размер таблицы

	# создание таблицы из нулевых значений
...
stuff = get_selected_items(stuffdict)
print(stuff)